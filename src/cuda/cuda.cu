...
int main(void) {
    ...
    blocks_per_grid.x = std::ceil(static_cast<double>(size) / static_cast<double>(threads_per_block.x));
    blocks_per_grid.y = std::ceil(static_cast<double>(size) / static_cast<double>(threads_per_block.y));

    dft_kernel << <blocks_per_grid, threads_per_block >> > (source_gpu, size, sum_gpu, result_gpu);

    cudaDeviceSynchronize();
    checkSync("Kernel launch");
